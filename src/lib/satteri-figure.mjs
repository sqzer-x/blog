 ...
 * image; 11 of the 24 image paragraphs in the corpus carry one (arp-spoofing 8, doh 2,
 * bind9 1). Without this they render as ordinary italic paragraphs and every image ships
 * as a bare <img> with no <figure>.
 *
 * It also repairs alt text: all 26 images in the posts have an alt that is empty or
 * filename-shaped ("image", "images", "img_4305", "2026-08-13-165932", "1"), which a screen
 * reader spells out letter by letter. Junk alt is cleared, and an image with a caption gets
 * an empty alt, because the figcaption beside it is already read out.
 *
 ...
 * carry exactly the same filename alts the corpus produces everywhere else. None of the 26
...
export default function satteriFigure() {
  return {
     ...
     * path did not claim -- plus the plates' own images, whose alt the figure path has
     * already emptied or kept as the author's own words, so testing it again changes
     * nothing. An alt is only ever cleared, never invented: a filename is worse than
     * silence to a screen reader, and this plugin has no way to know what the picture
     * shows.
     */
    image(node, ctx) {
      const alt = node.alt ?? '';
