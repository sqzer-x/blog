 ...
 *
 * Escaping is @astrojs/rss's, not ours. The corpus is full of the characters that break
 * hand-rolled XML: ampersands in shell pipelines, angle brackets in log output, quotes
 * and em-dashes in Korean prose.
 */
export async function GET(context: APIContext) {
  ...
  return rss({
    ...
    items: posts.map((p) => ({
      ...
      description: p.data.deck ?? excerpt(p.body ?? ''),
      link: postUrl(p),
    })),
